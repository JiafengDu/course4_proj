"""
  OMDb client follows these rules:
    Have a single method that's used to make requests,
  so we have just one place to add authentication and error handling.
    It should not need to know about Django at all,
  this will allow us to refactor more easily and re-use the code in a non-Django project.
  ...
  This means our API can change and only our client code needs to be updated,
  not different parsing code throughout our codebase.
"""
...
# Converting the movie's year into int
...
# this follows the idea of separation of concerns in software development
